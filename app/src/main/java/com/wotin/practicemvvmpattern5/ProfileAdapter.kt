...
class ProfileAdapter(private val context: Context) : RecyclerView.Adapter<ProfileAdapter.CustomViewHolder>() {
    ...
    class CustomViewHolder(val binding : RcvListItemBinding) : RecyclerView.ViewHolder(binding.root) {
        fun onBind(data : ProfileData) {
            if(data.name.startsWith("K")) data.profile = R.drawable.puffin  // K 로 시작하면 profile 의 resource 를 puffin 으로 변경.
            else if(data.name.startsWith("N")) data.profile = R.drawable.brain  // N 으로 시작하면 profile 의 resource 를 brain 으로 변경.
